...
{
    public class Detail_ListCarteTemporaireController : Controller
    {
        EPORTALEntities db = new EPORTALEntities();
        EPORTAL_NTEntities db_nt = new EPORTAL_NTEntities();
        PhanQuyenHTEntities dbP = new PhanQuyenHTEntities();
        int IDQuyenHT = EPORTAL.Models.MyAuthentication.IDQuyenHT;
        String controll = "Detail_ListCarteTemporaire";
        ...
        public ActionResult Index(int? id, int? page)
        {
            ...
            {
                return RedirectToAction("Index", "Detail_ViewTemporarycard", new { id = id });
            }*/
            if (page == null) page = 1;
            int pageSize = 50;
        ...
        public ActionResult Edit(NT_DetailCarteTemporaireValidation _DO, FormCollection collection)
        {
                ...
                try
                {
                 ...
                 //   var IDNV = db.NhanViens.Where(x => x.ID == id.IDNhanVien).FirstOrDefault();
                    ...
                    if (collection.Count == 8)
                    {
                            ...
                            if (dt.Rows.Count > 6)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    if (TTTID != 0)
                                    {
                                        //db_nt.NT_DetailCarteTemporaire_delete(TTTID);
                                        //db_nt.NT_CarteTemporaire_delete(TTTID);
                                    TempData["msgSuccess"] = "<script>alert('Vui lòng kiểm tra thông tin và import lại từ dòng : " + dtc + "');</script>";
                                    }
                                }
                                return RedirectToAction("Index", "Detail_ListCarteTemporaire", new { id = TTTID });
                        ...
                        {
                            TempData["msgSuccess"] = "<script>alert('Vui lòng chọn File Excel');</script>";
                            return RedirectToAction("Index", "Detail_ListCarteTemporaire", new { id = TTTID });
                        }    
                    }
                    else
            ...
            {
                TempData["msgSuccess"] = "<script>alert('Vui lòng kiểm tra lại dòng : " + dtc + "');</script>";
                return RedirectToAction("Index", "ListCarteTemporaire");
            }
            try
            { 
        ...
        public ActionResult EditChild(int id, int idttt)
        {
            ...
            return RedirectToAction("Index", "Detail_ListCarteTemporaire", new { id = idttt });
        }
        [HttpPost]
        public ActionResult EditChild(NT_DetailCarteTemporaireValidation request, int id)
        {
            if (request.HoVaTen != "" && request.CCCD != "" && request.LoaiPTID != null && request.ChucVu != "" && request.NgaySinh != null && request.Sdt != "" && request.BienSo != "" && request.Cong != 0 && request.SoPhieuDKVTTS != "" && request.VTTSChuaDK != "" && request.VTTheoPVCC != "" & request.SoLanRaVao != "")
            {
            ...
            return RedirectToAction("Index", "Detail_ListCarteTemporaire", new { id = request.IDTTT });
        }
