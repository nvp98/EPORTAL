...
{
    public class ListCarteTemporaireNTController : Controller
    {
        EPORTALEntities db = new EPORTALEntities();
        EPORTAL_NTEntities db_nt = new EPORTAL_NTEntities();
        PhanQuyenHTEntities dbP = new PhanQuyenHTEntities();
        int IDQuyenHT = EPORTAL.Models.MyAuthentication.IDQuyenHT;
        String controll = "ListCarteTemporaireNT";
        // GET: TagSign/ListCarteTemporaireNT
            ...
            try
            {
                    ...
                    if (key != "__RequestVerificationToken")
                    {
                        if (key == "HangMuc")
                        {
                            ...
                            if (ThoiHan < CheckDay)
                            {
                                ...
                                return RedirectToAction("Add", "ListCarteTemporaireNT");
                            }
                            if (collection["XacNhan_1"] == "Xác Nhận")
                            {
                                ...
                                db_nt.NT_CarteTemporaire_insert(collection["NoiDung"], Models.MyAuthentication.ID, DayUpload, 0, Convert.ToInt32(collection["IDNT"]), Convert.ToInt32(collection["IDPhongBan"]), collection["HangMuc"], ThoiHan, 1,0,filePath_1, returnIDTTT);
                            ...
                            try
                            {
                                for (int i = 6; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    if (HoVaTen != "")
                                    {
                                        ...
                                        //string MucDich = dt.Rows[i][4].ToString().Trim();
                                        /*if (MucDich == "")
                                        {
                                            TempData["msgSuccess"] = "<script>alert('Vui lòng nhập đầu đủ thông tin');</script>";
                                            return RedirectToAction("Index", "Detail_ListCarteTemporaireNT", new { id = TTTID });
                                        }*/
                                        /*var IDMD = db_nt.NT_Purpose.Where(x => x.TenMD == MucDich).FirstOrDefault();*/
                ...
                else
                {
                    ...
                    foreach (var item in ListCT)
                    {
                        ...
                        dtc=db_nt.NT_DetailCarteTemporaire.Where(x=>x.IDTTT==TTTID).ToList().Count +1;
                        if (CheckCCCD == null)
                        {
                        ...
                        {
                            TempData["msgError"] = "<script>alert('Xác nhận thành công');</script>";
                        }
                    }
                }
        ...
        public ActionResult CheckInformation(int id)
        {
            ...
            var KTV = (from au in db.AuthorizationContractors.Where(x => x.IDLKD == 2 && (x.NhanVien.IDPhongBan == Carte.PhongBanID || x.IDCVKN.Contains(Carte.PhongBanID.ToString())))
            ...
            VP1C.Add(new CheckInforUser() { HoTen = "Nhóm VP1C", IDNhanVien = 0 });
            ...
            try
            {
                ...
                if (idttt != null)
                {
                    if (carteTemp.TPBP != null&& carteTemp.VP1C!=null)
                    {
                        ...
                        if (carteTemp.KTV != null)
                        {
                            db_nt.sp_NTCarteTemp_Create(id, 1, null, carteTemp.KTV, null, carteTemp.GhiChu);
                        }

                        if (carteTemp.TPBP != null)
                        {
                            db_nt.sp_NTCarteTemp_Create(id, 2, null, carteTemp.TPBP, null, carteTemp.GhiChu);
                        }
                        if (carteTemp.TPBP != null)
                        {
                            db_nt.sp_NTCarteTemp_Create(id, 3, null, carteTemp.VP1C, null, carteTemp.GhiChu);                        
                        }
