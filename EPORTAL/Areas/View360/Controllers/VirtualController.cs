...
namespace EPORTAL.Areas.View360.Controllers
{
    public class VirtualController : Controller
    {
        ...
        public ActionResult Index(int? page, string search, string IDGroup)
        {
            ...
            var res = from a in db.Virtual_select(search)
            ...
            List<VirtualGroup> pg = db.VirtualGroups.ToList();
            if (IDGroup != "")
            {
        ...
        public ActionResult Authorization(int? page, int? id)
        {
            ...
            var model = db.Virtuals.Where(x => x.ID == id).FirstOrDefault();
        ...
        public ActionResult AddPermission(AuthorizationVirtualValidation _DO)
        {
            ...
            for (int i = 0; i < _DO.Selected.Length; i++)
            {
                ...
                aus.VirtualID = Convert.ToInt32(_DO.ID);
                aus.Createdate = DateTime.Now;
                db.AuthorizationVituals.Add(aus);
                db.SaveChanges();
            }
            return RedirectToAction("Authorization", "Virtual", new { id = _DO.ID });
        ...
        public JsonResult DeleteAutho(string[] IDdata)
        {
            ...
            try
            {
                ...
                {
                    db.AuthorizationVitual_delete(Convert.ToInt32(data));
                }
        ...
        {
            // Server.MapPath -> duong dan vat ly duoi web-app root (tranh DirectoryNotFound).
        ...
        public ActionResult ImportExcel()
        {
            ...
            List<ProjectView> pro = db.Virtuals.Select(x => new ProjectView { ID = x.ID, Title = x.Title }).ToList();
                        ...
                        try
                        {
                            for (int a = 0; a < _DO.Selected.Length; a++)
                            {
                                ...
                                for (int i = 2; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    var NhanVien = db.AuthorizationVituals.Where(x => x.VirtualID == IDProject && x.NhanVienID == IDNV).FirstOrDefault();
                                    if (IDNV > 0 && CheckPer(IDNV, _DO.VirtualID) == 0 && NhanVien == null)
                                    {
                                        aus.NhanVienID = IDNV;
                                        aus.VirtualID = Convert.ToInt32(_DO.Selected[a]);
                                        aus.Createdate = DateTime.Now;
                                        db.AuthorizationVituals.Add(aus);
                                        db.SaveChanges();
                                        dtc++;
        ...
        {
            var Regsbb = (from u in db.AuthorizationVituals
                          where u.NhanVienID == NhanVienID && u.VirtualID == ProjectID
                          select new { u.ID }).FirstOrDefault();
            if (Regsbb != null)
                return Regsbb.ID;
            return 0;
        ...
        public ActionResult ExportToExcel(String search, string IDGroup)
        {
            ...
            try
            {
                ...
                List<VirtualGroup> listpg = db.VirtualGroups.ToList();
                //IList<Project> list_Projects = db.Projects.ToList();
                var list_Projects = (from a in db.Virtual_select(search)
                                     select new VirtualValidation
                                     {
                                         ID = a.ID,
                                         Title = a.Title,
        ...
        public ActionResult ExportToExceldetail(int id)
        {
            ...
            try
            {
                ...
                {
                    TempData["msg"] = "<script>alert('Không có dữ liệu');window.location.href = '/Virtual'</script>";
                    return RedirectToAction("Index", "Virtual");
                }
            ...
            {
                TempData["msg"] = "<script>alert('" + ex + "');window.location.href = '/Virtual'</script>";
                return RedirectToAction("Index", "Virtual");
            }
